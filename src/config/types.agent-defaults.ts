...
export type AgentDefaultsConfig = {
  ...
  heartbeat?: {
    ...
    accountId?: string;
    /** Override the heartbeat prompt body (default: "Read HEARTBEAT.md if it exists (workspace context). Follow it strictly. Do not infer or repeat old tasks from prior chats. If nothing needs attention, reply PULSE_ACK."). */
    prompt?: string;
    /** Include the ## Heartbeats system prompt section for the default agent (default: true). */
    includeSystemPromptSection?: boolean;
    /** Max chars allowed after PULSE_ACK before delivery (default: 30). */
    ackMaxChars?: number;
...
export type AgentCompactionConfig = {
  ...
  model?: string;
  /** Optional per-call reasoning effort for CLI-backend compaction summaries.
   * Overrides a backend-pinned effort for the summary turn only; unset inherits
   * the active backend effort. */
  effort?: "off" | "minimal" | "low" | "medium" | "high" | "xhigh" | "adaptive" | "max";
...
export type AgentCompactionMemoryFlushConfig = {
  ...
  model?: string;
  /** Optional per-call reasoning effort for CLI-backend memory-flush turns.
   * Overrides a backend-pinned effort for the flush turn only; unset inherits
   * the active backend effort. */
  effort?: "off" | "minimal" | "low" | "medium" | "high" | "xhigh" | "adaptive" | "max";
