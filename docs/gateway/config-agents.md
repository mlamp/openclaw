...
- `effort` (fork): reasoning effort for OpenClaw-triggered CLI compaction and manual native `/compact`; unset inherits session thinking. Accepts `off`, `minimal`, `low`, `medium`, `high`, `xhigh`, `adaptive`, or `max`. Native mid-turn auto-compaction remains runtime-owned.
- `memoryFlush.effort` (fork): independent reasoning effort for CLI memory extraction, with the same accepted values; unset inherits backend effort. CLI extraction uses the compaction `timeoutSeconds` budget.
